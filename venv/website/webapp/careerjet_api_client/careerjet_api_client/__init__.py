    ...
    def search(self, search_params):
        ...
        for field in Constants.MANDATORY_FIELDS:
            if not field in search_params or not search_params[field]:
                raise Exception('Mandatory param key \'' +
                                field + '\' missing')
        ...
        if search_params['locale_code'] not in Constants.LOCALES:
            raise Exception(
                'Locale '+search_params['locale_code']+' not supported')

        referer_uri = urlparse(search_params.pop('url'))
        if referer_uri.scheme != 'http' and referer_uri.scheme != 'https':
            raise Exception('Invalid param url \'' +
                            search_params['url'] + '\'')

        user_agent = 'careerjet-api-client-v' + pkg_resources.require(
            "careerjet_api_client")[0].version + '-python-v'+platform.python_version()

        try:
            response = requests.get(Constants.API_URL + '/search', headers={
                                    'user-agent': user_agent, 'referer': referer_uri.geturl()}, params=search_params)
